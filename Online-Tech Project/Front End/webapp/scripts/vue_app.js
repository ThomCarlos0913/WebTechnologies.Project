...
var index_vue = new Vue({
  ...
  mounted() {
    this.showslide()
    axios.get("http://localhost:8000/get_passed_events")
    .then(response => {this.list_passed = response.data})
    axios.get("http://localhost:8000/get_featured")
...
var login_vue = new Vue ({
  ...
  methods: {
    ...
    verifyaccount: function() {
      axios.get('http://localhost:8000/validate_account', {
        auth: {
    ...
    registeraccount: function() {
      if (this.signup_username && this.signup_password && this.signup_confirmpass && this.signup_email){
          if(this.signup_password == this.signup_confirmpass){
            axios.post('http://localhost:8000/register_acount', {
...
var admin_vue = new Vue ({
  ...
  methods: {
    ...
    create_event: function() {
      if (this.event_name && this.event_location && this.event_time && this.event_details) {
        axios.post('http://localhost:8000/create_event', {
    ...
    update_event: function() {
      axios.get('http://localhost:8000/update_event', {
        params: {
          id: this.update_id,
    ...
    delete_event: function() {
      axios.get('http://localhost:8000/delete_event', {
        params: {
          id: this.update_id
        }
      })
    ...
    update_featured: function() {
      axios.get('http://localhost:8000/update_featured', {
        params: {
          id: this.update_id
        }
      })
  ...
  mounted() {
    axios.get('http://localhost:8000/get_events')
  ...
  methods: {
    ...
    updateeventpanel: function(n) {
      ...
      if (this.event_panel == 1) {
        axios.get('http://localhost:8000/get_upcoming_event')
        .then(response => {this.event_template = response.data;})
      }
      if (this.event_panel == 2) {
        axios.get('http://localhost:8000/get_my_events', {
          params: {
    ...
    subscribeevent: function(key, title, time, venue, details) {
      if(this.t_taken) {
        axios.post('http://localhost:8000/subscribe_event', {
    ...
    unsubscribe: function(key) {
      axios.get("http://localhost:8000/unsubscribe_event", {
          params: {
